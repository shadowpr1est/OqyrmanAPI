...
func TestUpsert_DowngradesFalseFinishedWhenProgressBelow100(t *testing.T) {
	repo := new(mockSessionRepo)
	uc := readingSession.NewReadingSessionUseCase(repo)
...
func TestUpsert_ClampsProgressOutsideRange(t *testing.T) {
	repo := new(mockSessionRepo)
	uc := readingSession.NewReadingSessionUseCase(repo)

	high := &entity.ReadingSession{Progress: 250, Status: entity.StatusReading}
	repo.On("Upsert", mock.Anything, high).Return(high, nil)
	_, err := uc.Upsert(context.Background(), high)
	assert.NoError(t, err)
	assert.Equal(t, 100, high.Progress)

	low := &entity.ReadingSession{Progress: -10, Status: entity.StatusReading}
	repo.On("Upsert", mock.Anything, low).Return(low, nil)
	_, err = uc.Upsert(context.Background(), low)
	assert.NoError(t, err)
	assert.Equal(t, 0, low.Progress)
}
